...
def _write_dump(path: Path, biomes: dict, mods: list[str] | None = None) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps({"mods": mods or [], "biomes": biomes}), encoding="utf-8")


def _setup(monkeypatch, tmp_path: Path, major: str = "2.8") -> Path:
...
def test_instance_dump_overrides_bundled(monkeypatch, tmp_path):
    _setup(monkeypatch, tmp_path)
...
def test_no_dump_anywhere_returns_empty(monkeypatch, tmp_path):
