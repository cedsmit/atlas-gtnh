...
def _patch_modlist(monkeypatch: pytest.MonkeyPatch, mods: dict[str, str]) -> None:
    monkeypatch.setattr(pack_version, "read_world_modlist", lambda _p: mods)


def test_no_signatures_returns_none(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_modlist(monkeypatch, {"gregtech_nh": "5.09.51.470"})
    assert pack_version.detect_gtnh_major("w", {}) is None


def test_picks_matching_major(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_modlist(monkeypatch, {"gregtech_nh": "5.09.51.470", "gtnhlib": "0.6.40"})
    assert pack_version.detect_gtnh_major("w", SIGS) == "2.8"


def test_picks_newer_major(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_modlist(monkeypatch, {"gregtech_nh": "5.10.00.999", "gtnhmixins": "2.4.0"})
    assert pack_version.detect_gtnh_major("w", SIGS) == "2.9"


def test_anchor_match_is_case_insensitive(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_modlist(monkeypatch, {"GregTech_NH": "5.09.51.470"})
    assert pack_version.detect_gtnh_major("w", SIGS) == "2.8"


def test_empty_modlist_falls_back_to_newest(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_modlist(monkeypatch, {})
    assert pack_version.detect_gtnh_major("w", SIGS) == "2.9"


def test_unknown_versions_fall_back_to_newest(monkeypatch: pytest.MonkeyPatch) -> None:
    _patch_modlist(monkeypatch, {"gregtech_nh": "99.99"})  # matches neither major
    assert pack_version.detect_gtnh_major("w", SIGS) == "2.9"


def test_single_major_always_that_major(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_load_major_signatures(tmp_path: Path) -> None:
    d = tmp_path / "2.8"
    d.mkdir()
...
def test_load_major_signatures_missing_dir(tmp_path: Path) -> None:
