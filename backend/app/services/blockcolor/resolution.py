...
# {mc_dir}/config/atlas/icon_dump.json.  Resolution order (most specific first):
...
#   4. backend/app/data/<major>/icon_dump.json.gz  (canonical dump bundled with
#      Atlas, version-selected from the world's ModList — the zero-setup default)
# Forgiving: if no dump is found at all we don't record the attempt, letting a
# later-dropped file be picked up on the next world access without a restart.

# Canonical dumps ship under backend/app/data/<major>/ (this file is at
...
def _bundled_icon_dump(world_path: str | None) -> Path | None:
    """The version-selected bundled icon dump for this world, or None.

    Detects the GTNH major version from the world's ModList (reusing the biome
    dump's ``mods`` signature, which ships uncompressed) and returns
    ``data/<major>/icon_dump.json.gz`` (falling back to an unzipped
    ``icon_dump.json``).
    """
    if not world_path:
        return None
    ...
    if not major:
        return None
    for name in ("icon_dump.json.gz", "icon_dump.json"):
        cand = _DATA_DIR / major / name
        if cand.exists():
            return cand
    return None
...
def _load_asset_db(world_path: str) -> AssetDatabase:
    ...
    _try_auto_load_dump(mc_dir, world_path)
    return db
