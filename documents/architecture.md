...
| `services/color_cache.py` | SQLite cache of scanned texture colors / JSON assets (raw sqlite3) |
| `world/` | Hand-rolled `.mca` region I/O (nbtlib + numpy) |

**Key libraries:**
- **Hand-rolled MCA reader/writer** — reads and writes Anvil `.mca` region files directly, including GTNH's extended (NEID `Blocks16`) block IDs
- **NBTLib + NumPy** — NBT parsing and vectorized section/block decode
- **Raw `sqlite3`** — lightweight derived-metadata cache (no ORM)
...
3. Backend scans .mca files via the hand-rolled region reader
...
3. The region writer serialises each chunk
