...
async function fetchChunkData(worldPath: string, cx: number, cz: number): Promise<ChunkData> {
  const res = await fetch(
    `${API_BASE}/worlds/chunks/${cx}/${cz}?world_path=${encodeURIComponent(worldPath)}`
  )
  if (!res.ok) throw new Error(`Failed to load chunk (${cx}, ${cz}): ${res.statusText}`)
