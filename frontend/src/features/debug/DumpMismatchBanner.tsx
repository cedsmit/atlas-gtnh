...
import { columnTally } from '../map/columnTally'
...
const STYLES = {
  error: {
    box: 'border-red-700/60 bg-red-950/90',
    dot: 'bg-red-500',
    title: 'text-red-200',
  },
  warn: {
    box: 'border-amber-700/60 bg-amber-950/90',
    dot: 'bg-amber-500',
    title: 'text-amber-200',
  },
  info: {
    box: 'border-sky-800/60 bg-sky-950/90',
    dot: 'bg-sky-500',
    title: 'text-sky-200',
  },
} as const
...
export function DumpMismatchBanner({ worldPath }: Props) {
  ...
  const missingMods = (data.missing_from_dump ?? []).filter(
    (m) => m.block_count > 0
  )
  ...
  const handleExport = async (format: 'json' | 'csv') => {
    ...
    setExporting(format)
    try {
      await downloadMissingBlockReport(
        worldPath,
        format,
        columnTally.snapshot()
      )
    } catch (e) {
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          <div
            className={`inline-flex items-center gap-1.5 font-semibold ${s.title}`}
          >
            <TriangleAlert className="h-4 w-4 shrink-0" aria-hidden />
            {title}
          </div>
          <div className="mt-0.5 text-zinc-400">
            World has {data.world_mod_count} mods; dump has{' '}
            {data.dump_mod_count}.
            {missingTotal > 0 && (
              <>
                {' '}
                {missingTotal} world block{missingTotal === 1 ? '' : 's'} absent
                from dump
                {driftTotal > 0 && (
                  <>
                    {' '}
                    ({driftTotal} are registration drift — mod present, block
                    not dumped)
                  </>
                )}
                .
          ...
          {visibleMissing.length > 0 && (
            <div className="mt-1.5">
              <div className="text-zinc-500">
                On the map ({visibleMissing.length} block
                {visibleMissing.length === 1 ? '' : 's'}):
              </div>
              <ul className="mt-0.5 space-y-0.5">
                ...
                {visibleMissing.length > shownBlocks.length && (
                  <li className="text-zinc-500">
                    +{visibleMissing.length - shownBlocks.length} more on the
                    map
                  </li>
                )}
              </ul>
              ...
              <ul className="mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5">
                {missingMods.slice(0, 8).map((m) => (
                  <li key={m.mod_id} className="font-mono text-red-300">
                    {m.mod_id}{' '}
                    <span className="text-zinc-500">({m.block_count} blk)</span>
                  </li>
                ))}
                {missingMods.length > 8 && (
                  <li className="text-zinc-500">
                    +{missingMods.length - 8} more
                  </li>
                )}
              </ul>
            </div>
          ...
          {versions.length > 0 && (
            <div className="mt-1.5">
              ...
              <ul className="mt-0.5 space-y-0.5">
                ...
                {versions.length > 4 && (
                  <li className="text-zinc-500">+{versions.length - 4} more</li>
                )}
              </ul>
            </div>
          )}
          ...
          <div className="mt-2 text-zinc-500">
            ...
            {exporting && (
              <span className="ml-1 text-zinc-400">generating…</span>
            )}
          </div>
        </div>
