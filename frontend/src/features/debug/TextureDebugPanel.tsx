...
export function TextureDebugPanel({ worldPath, registry, onClose }: Props) {
  ...
  const [pipelineReport, setPipelineReport] = useState<PipelineReport | null>(
    null
  )
  ...
  // These read the debug store imperatively; `tick` (bumped when the store or
  // texture states change) is the recompute trigger, so it is an intentional dep.
  /* eslint-disable react-hooks/exhaustive-deps */
  const allBlocks = useMemo(() => textureDebugStore.getAll(), [tick])
  const stats = useMemo(() => textureDebugStore.getStats(), [tick])
  const renderTotals = useMemo<RenderTotals>(
  ...
  async function traceResolution() {
    ...
    try {
      ...
      console.log(
        'Vanilla keys in scanned colors:',
        data.vanilla_keys_in_colors
      )

      console.group('JARs found:')
      for (const jar of data.jars ?? []) {
        console.log(
          `  ${jar.status === 'cached' ? '📦' : jar.status === 'scanned' ? '🔍' : '❌'} ${jar.jar} — ${jar.keys} keys (${jar.status})`
        )
      }
      console.groupEnd()
      ...
      console.group(
        `Block resolution: ${fromJar.length} from JAR, ${fallback.length} from fallback table, ${noMapping.length} unresolved`
      )
      for (const b of noMapping) {
        console.log(
          `%c? [${b.id}] ${b.name} — no key (neither JAR nor fallback table)`,
          'color:#f59e0b'
        )
      }
      for (const b of fallback) {
        console.log(
          `%c~ [${b.id}] ${b.name} → ${b.fallback_key} (fallback — image may 404 if JAR not found)`,
          'color:#a78bfa'
        )
      }
      console.groupEnd()
  ...
  return (
    <div className="flex h-full w-[480px] shrink-0 flex-col border-l border-zinc-800 bg-atlas-row">
      {/* Header */}
      ...
      <div className="flex flex-col gap-1 border-b border-zinc-800 px-4 py-2 font-mono text-xs">
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-0.5">
          ...
          <span className="ml-auto text-zinc-600">
            {stats.total} unique block types
          </span>
        </div>
        ...
        {stats.occLoaded + stats.occMissing + stats.occNoMapping > 0 && (
          <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-[10px]">
            <span className="text-emerald-600">
              {stats.occLoaded.toLocaleString()} cols w/ tex
            </span>
            <span className="text-red-700">
              {stats.occMissing.toLocaleString()} cols failed
            </span>
            <span className="text-amber-700">
              {stats.occNoMapping.toLocaleString()} cols no-key
            </span>
            {(() => {
              const total =
                stats.occLoaded +
                ...
                stats.occPending
              const pct =
                total > 0 ? Math.round((stats.occLoaded / total) * 100) : 0
              return (
                <span className="ml-auto text-zinc-600">{pct}% textured</span>
              )
            })()}
          </div>
        )}
...
function PipelineReportPanel({
  ...
  const categories: PipelineCategory[] = Object.entries(report.categories).map(
    ([cat, count]) => ({
      category: cat,
      count,
    ...
    <div className="border-b border-indigo-900 bg-indigo-950/30 px-4 py-3 font-mono text-xs">
      ...
      <div className="mb-1.5 flex flex-wrap gap-x-4 gap-y-0.5 text-[11px]">
        <span className="text-emerald-400">
          {report.pipeline_resolved} resolved
        </span>
        <span className="text-amber-400">
          {report.pipeline_unresolved} unresolved
        </span>
        <span className="ml-auto text-indigo-500">
          {resolvedPct}% of {report.total} blocks
        </span>
      </div>
      {/* Per-stage breakdown */}
      <div className="mb-1 flex flex-wrap gap-x-3 gap-y-0.5 text-[10px]">
        {report.override_resolved > 0 && (
          <span className="text-violet-400">
            {report.override_resolved} override
          </span>
        )}
        {report.forge_dump_resolved + report.forge_dump_ambiguous > 0 && (
          <span className="text-sky-400">
            {report.forge_dump_resolved + report.forge_dump_ambiguous} forge
            dump
            {report.forge_dump_ambiguous > 0 && (
              <span className="text-orange-400">
                {' '}
                ({report.forge_dump_ambiguous} ~)
              </span>
            )}
          </span>
        )}
        ...
        {report.forge_dump_loaded ? (
          <div className="flex flex-wrap gap-x-3 gap-y-0.5">
            <span className="inline-flex items-center gap-1 text-sky-400">
              <Check className="h-3.5 w-3.5" aria-hidden /> Forge dump loaded
            </span>
            <span className="text-zinc-500">
              {report.forge_dump_block_count.toLocaleString()} blocks
            </span>
            {report.forge_dump_path && (
              <span
                className="truncate text-zinc-600"
                title={report.forge_dump_path}
              >
          ...
          <span className="inline-flex items-center gap-1 text-amber-500">
            <X className="h-3.5 w-3.5 shrink-0" aria-hidden /> Forge dump not
            loaded — build &amp; install AtlasDumper mod, run GTNH once
          </span>
        )}
      </div>
      ...
      {report.legacy_resolved > 0 && (
        <div className="mb-2 flex flex-wrap gap-x-3 gap-y-0.5 pl-2 text-[10px]">
          ...
          {report.legacy_ambiguous > 0 && (
            <span className="text-orange-400">
              {report.legacy_ambiguous} ambiguous
            </span>
          )}
        </div>
      )}
        ...
        {categories.map(({ category, count, examples }) => (
          <details key={category} className="group">
            <summary className="flex cursor-pointer items-baseline gap-2 list-none">
              ...
              <ChevronRight
                className="ml-auto h-3.5 w-3.5 text-zinc-600 group-open:hidden"
                aria-hidden
              />
              <ChevronDown
                className="ml-auto hidden h-3.5 w-3.5 text-zinc-600 group-open:inline"
                aria-hidden
              />
            </summary>
            {examples.length > 0 && (
              <div className="ml-4 mt-1 flex flex-col gap-0.5">
                {examples.map((ex) => (
                  <span key={ex} className="truncate text-[10px] text-zinc-500">
                    {ex}
                  </span>
                ))}
              </div>
      ...
      {report.legacy_examples &&
        Object.keys(report.legacy_examples).length > 0 && (
          <div className="mt-2 flex flex-col gap-1 border-t border-indigo-900 pt-2">
            ...
            {Object.entries(report.legacy_examples).map(([tag, exs]) => (
              <details key={tag} className="group">
                <summary className="flex cursor-pointer items-baseline gap-2 list-none">
                  <span
                    className={`rounded px-1 font-mono text-[9px] ${METHOD_CLASS[tag] ?? 'bg-zinc-800 text-zinc-400'}`}
                  >
...
function RenderProof({ rt }: { rt: RenderTotals }) {
  ...
  return (
    ...
    >
      {/* Headline */}
      <div
        className={`inline-flex items-center gap-1.5 font-semibold ${isWorking ? 'text-emerald-400' : 'text-red-400'}`}
      >
        {isWorking ? (
      ...
      <div className="mt-1 flex flex-wrap gap-x-4 gap-y-0.5 text-[11px]">
        <span className="text-emerald-400">
          drawImage: {rt.drawImage.toLocaleString()}
        </span>
        <span className="text-zinc-400">
          fillRect: {rt.fillRect.toLocaleString()}
        </span>
        <span className="text-amber-400">
          no-key: {rt.missingTexKey.toLocaleString()}
        </span>
        <span className="text-red-400">
          failed-load: {rt.failedTexLoad.toLocaleString()}
        </span>
...
function DebugRow({
  ...
  async function traceBlockPipeline() {
    ...
    try {
      ...
      for (const step of (data.trace ?? []) as {
        ok: boolean
        step: string
      }[]) {
      ...
      if (data.resolved) {
        ...
        if (candidates.length > 0) {
          ...
          candidates.forEach((c, i) => {
            ...
            console.log(
              `  ${i + 1}. ${c.key}  score=${c.score}${c.notes ? `  ${c.notes}` : ''}  ${marker}`
            )
          })
          console.groupEnd()
        }
  ...
  return (
    <div className="flex flex-col border-b border-zinc-900 px-3 py-1.5">
      <div className="flex items-center gap-2">
        ...
        {/* ID */}
        <span className="w-8 shrink-0 font-mono text-[11px] text-zinc-500">
          {b.id}
        </span>
        ...
        <span className="w-16 shrink-0">
          {tintLabel ? (
            <span
              className={`rounded px-1 py-0.5 font-mono text-[9px] ${tintClass}`}
            >
              {tintLabel}
            </span>
          ) : blockMethod ? (
            <span
              className={`rounded px-1 py-0.5 font-mono text-[9px] ${METHOD_CLASS[blockMethod] ?? 'bg-zinc-800 text-zinc-500'}`}
            >
        ...
        {/* Status */}
        <span
          className={`w-16 shrink-0 font-mono text-[10px] ${STATUS_CLASS[status]}`}
        >
          {STATUS_LABEL[status]}
        </span>
...
function buildSuggestedJson(b: DebugBlockView, knownCategory?: string): string {
  ...
  const tint =
    b.tintType !== 'none' && b.tintType !== 'water'
      ? `, "tint": "${b.tintType}"`
      : ''
