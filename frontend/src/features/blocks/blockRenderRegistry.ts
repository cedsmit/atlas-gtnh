...
export type RenderCategory = 'solid' | 'overlay' | 'fluid' | 'transparent' | 'partial' | 'ignore'
...
export function isVisibleInMode(def: ResolvedDefinition, mode: MapMode): boolean {
...
export class BlockRenderRegistry {
  ...
  private readonly byName = new Map<string, { def: BlockRenderDefinition; source: string }>()
  ...
  resolveNames(blockNames: Record<number, string>): void {
    for (const [rawId, name] of Object.entries(blockNames)) {
      ...
      if (entry) {
        this.byId.set(Number(rawId), { ...entry.def, resolverSource: entry.source })
      }
    }
  }
